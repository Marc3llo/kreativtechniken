...
var showgrid=!1,pixelgrid=!1;$(document).ready(function(){1==showgrid&&$("html").addClass("showgrid"),1==pixelgrid&&$("html").addClass("pixelgrid"),$.getJSON("models/_global.json",function(t){$(document).on("click","#signinbtn",function(){event.preventDefault();var e=$("input#password").val(),i=$("input#user").val();$.each(t.users,function(){if(password=this.password,username=this.username,e==password&&i==username){var t={user:username};localStorage.setItem("signin",JSON.stringify(t)),localStorage.getItem("signin"),window.location.reload()}else console.log("nope")})});var e=localStorage.getItem("signin"),i=JSON.parse(e);e&&($.each(t.users,function(){username=this.username,i.user==username&&$("body").addClass(username)}),$("body").addClass("signedin"))}),$(document).on("click","#signout",function(){console.log(this),localStorage.clear("signin"),window.location.reload()})});